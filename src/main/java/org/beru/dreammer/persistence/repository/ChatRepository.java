...
public interface ChatRepository extends ListCrudRepository<ChatEntity, String>{
    @Query(value = "SELECT c.* FROM chats c JOIN users_chats uc1 ON c.id = uc1.id JOIN users_chats uc2 ON c.id = uc2.id WHERE uc1.username = ?1 AND uc2.username = ?2",nativeQuery = true)
